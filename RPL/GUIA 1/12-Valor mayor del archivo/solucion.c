/*
Ya estamos a punto de terminar el repaso de C, lo único que nos queda es cómo interactuar con archivos.
 Dado el nombre del archivo, se pide que se lean los números almacenados en él y se devuelva el más grande.

El archivo tiene la siguiente estructura un_numero;otro_numero;y_otro_numero;\n; el caracter \n es un salto de línea. 
Entonces, los números almacenados en .txt están separados por ; y no hay espacios entre ellos. 
Cuando se encuentra con un \n es porque ya no hay más elementos. No se sabe de antemano cuántos números hay en el archivo.
 Se puede asumir que el archivo tendrá como mínimo un elemento.

No te preocupes por el tipo de dato FILE*, lo único que tenés que saber es que para usar las funciones de lectura de archivos, 
tenés que recibir ese tipo de dato.
...
Unamos todos estos puntos. Para leer el archivo, podría usar esa estructura iterativa… pero, 
cuál sería la condición de corte? 
Sabiendo que la función fscanf devuelve la cantidad de elementos que leyó, qué pasa si lee menos de las esperadas? 
*/
#include <stdio.h>
